...
class Vehicle
  ...
  def calculate_age(when_bought_vehicle)
    time_now = Time.now
    age_math(time_now, when_bought_vehicle)
  end

  private

  def age_math(time_now, when_bought_vehicle)
    puts "Your vehicle is #{time_now - when_bought_vehicle} old in seconds."
  end
end
...
when_bought_vehicle = Time.utc(2011,4,8,8,50,26)
car1.calculate_age(when_bought_vehicle)
